...
### 5. RAG 管理（入库 + 分配有效时间 + 重建索引）

rag-manager 负责入库爬虫产物、给通知分配有效时间（过期自动从查询排除）、重建索引。

```bash
# 一条命令：入库爬虫产物 → 判有效期 → 重建索引 → 询问清理已收录的爬虫产物
cd rag-manager && python rag_manager.py --ingest

# 或：通过查询链自动触发（run_crawler 入库 + 触发 rag-manager）
cd query-agent && python query.py "cs.nju.edu.cn 最近有什么通知"

# 或：只处理已入库的 pending（不 ingest）
...
- **图片/视频/音频**：爬虫**只提取文本正文**，**不抓取、不存储图片/视频/音频文件**。附件 URL（如 PDF）会标注在内容里（"正文见附件：...pdf"）；图片/视频内容需到原网页查看。
- **正文形态**：部分通知正文承载于 PDF/Flash 播放器或为纯图片（如学术报告海报），此类页面正文文本较短，系统会标注附件链接而非正文全文。
- **需登录站点**：SSO 站点（如 ndwy）需浏览器模式（`--browser`）扫码登录后爬取，纯 HTTP 无法访问。**需统一身份认证网站的爬取设计尚不稳定，有概率失败**（依赖扫码登录环境，未端到端实测）。
- **JS 动态渲染**：正文由 JS 动态加载的页面，HTTP 爬取可能只得到骨架文本；此类站点建议浏览器模式。
- **精细调控爬取策略**：若需精细调控某站点爬取策略（入口/分页/提取规则），**请使用 explorer-agent**（`cd explorer-agent && python main.py "探索 <站点URL/域名>"`，如 `python main.py "探索 https://med.nju.edu.cn/"`）——交互式可获得更细粒度控制；分发链的 `run_explorer` 是自动化草稿生成，控制粒度有限。
