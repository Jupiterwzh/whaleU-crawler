...
37. **待办防遗忘** — 用户一次提出多条任务、我只处理其中部分时，把**未处理项**写入本地待办文件（含上下文污染风险：分批次处理时，未办事项若不记录，后续可能遗忘）。每个待办项需足够具体（含做什么/为什么/涉及文件），完成即勾掉。
38. **大作业规范源** — 大作业要求以项目外规范文档为准（老师提供的通用要求与应用类项目说明）；涉及交付、流程、凭据、CI、分发、日志或文档时，必须先对照原文，不只依赖摘要。
39. **文档同步与教程** — 每次有意义改动后，及时同步 `AGENT_LOG.md`、相关 `SPEC.md`/`PLAN.md`/`README.md`/验收清单，并更新面向用户的教程或清单；不得把已完成事项继续留在临时待办中。
