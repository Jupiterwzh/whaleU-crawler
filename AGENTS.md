...
20. **默认 subagent-driven 执行** — 有实现计划时，按 subagent-driven-development skill 每 Task 派一个新 subagent，Task 间 controller 复核。**执行方式（subagent-driven vs inline）默认 subagent-driven，无需询问。**
...
37. **待办防遗忘** — 用户一次提出多条任务、我只处理其中部分时，把**未处理项**写入 `无关文档/本地工具/待办.md`（含上下文污染风险：分批次处理时，未办事项若不记录，后续可能遗忘）。每个待办项需足够具体（含做什么/为什么/涉及文件），完成即勾掉。
38. **大作业规范源** — 大作业要求以 `无关文档/通用要求.md` 与 `无关文档/AI4SE_Final_Project_应用类项目.md` 为准；涉及交付、流程、凭据、CI、分发、日志或文档时，必须先对照原文，不只依赖摘要。
39. **文档同步与教程** — 每次有意义改动后，及时同步 `AGENT_LOG.md`、相关 `SPEC.md`/`PLAN.md`/`README.md`/验收清单，并更新面向用户的教程或清单（主要是 `无关文档/`、`无关文档/本地工具/待办.md`）；不得把已完成事项继续留在临时待办中。
