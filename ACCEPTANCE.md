...
## 测试 3：RAG 管理 Agent 入库 + 分析有效期（去重）

**方式 A（推荐）：rag-manager 一条命令入库 + 判有效期**
```bash
cd rag-manager
python rag_manager.py --ingest
# 入库 crawler/data 的 notices_*.jsonl（内容去重）→ 判有效时间 → 重建索引 → 询问清理已收录产物
```

**方式 B：通过 query-agent 分发链（自动 ingest + 触发 rag-manager）**
```bash
cd query-agent
